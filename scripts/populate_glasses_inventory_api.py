#!/usr/bin/env python3
"""
Populate Glasses Inventory via API
This script populates the glasses inventory using the working API endpoints.
"""
...
class GlassesInventoryPopulator:
    ...
    async def get_sample_inventory_items(self) -> List[Dict[str, Any]]:
        """Get sample glasses inventory items"""
        return [
            {
            ...
            async with self.session.get(
                ...
                if response.status == 200:
                    ...
                    if items:
                        ...
                        print(f"   Total stock value: {sum(item.get('stock_quantity', 0) * item.get('price', 0) for item in items):.2f} THB")
                    return True
                else:
...
async def main():
    """Main function"""
    print("👓 EVEP Medical Portal - Populate Glasses Inventory")
    print("=" * 60)
