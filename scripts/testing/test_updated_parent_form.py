#!/usr/bin/env python3
"""
Test script to verify updated Edit Parent form (income_level removed, profile_photo added)
"""
import asyncio
import aiohttp
import json

async def test_updated_parent_form():
    """Test the updated parent form data population"""
        ...
        try:
            ...
            async with session.get(f"http://localhost:8014/api/v1/evep/parents/{parent_id}", headers=headers) as response:
                if response.status == 200:
                    ...
                    print(f"   Profile Photo: {parent_data.get('profile_photo', 'Not set')[:50]}...")
                    ...
                    # Test 2: Check if form can be populated (updated fields)
                    print("\n🔍 Testing updated form data population...")
                    form_data = {
                        ...
                        "profile_photo": parent_data.get("profile_photo", ""),
                        "address": parent_data.get("address", {}),
                        "emergency_contact": parent_data.get("emergency_contact", {})
                    }
                    
                    print("✅ Updated form data structure created successfully!")
                    print(f"   Form data keys: {list(form_data.keys())}")
                    
                    # Test 3: Verify income_level is removed
                    if 'income_level' in parent_data:
                        print("⚠️  income_level field still present in API response")
                    else:
                        print("✅ income_level field successfully removed from API response")
                    
                    # Test 4: Verify profile_photo is present
                    if 'profile_photo' in parent_data:
                        print("✅ profile_photo field successfully added to API response")
                    else:
                        print("⚠️  profile_photo field missing from API response")
                    
                    # Test 5: Verify address structure
                    ...
                    # Test 6: Verify emergency contact structure
                    ...
                    # Test 7: Check if all fields match frontend expectations (updated)
                    print("\n🔍 Checking updated field compatibility with frontend...")
                    frontend_expected_fields = [
                        'first_name', 'last_name', 'cid', 'birth_date', 'gender', 'phone', 'email',
                        'relation', 'occupation', 'profile_photo', 'address', 'emergency_contact'
                    ]
                    ...
                    # Test 8: Verify income_level is not in frontend expected fields
                    if 'income_level' in frontend_expected_fields:
                        print("⚠️  income_level still in frontend expected fields")
                    else:
                        print("✅ income_level successfully removed from frontend expected fields")
                    
                    print("\n🎉 Updated Edit Parent Form Test Summary:")
                    print("✅ API returns parent data correctly")
                    print("✅ All required fields are present")
                    print("✅ Form data structure is complete")
                    print("✅ income_level field successfully removed")
                    print("✅ profile_photo field successfully added")
...
if __name__ == "__main__":
    asyncio.run(test_updated_parent_form())
