...
class PatientsCollectionPopulator:
    def __init__(self):
        self.session = None
    ...
    async def create_patient_record(self, student: Dict[str, Any]) -> bool:
        """Create a patient record using the patients API"""
        try:
            headers = {"Authorization": f"Bearer {self.access_token}"}
            ...
            patient_data = {
                ...
                "gender": "male" if student.get("gender") == "1" else "female",
                "parent_email": f"parent.{student.get('student_code', '').lower()}@evep.com",
                "parent_phone": "081-000-0000",
                "emergency_contact": "Parent/Guardian",
                "emergency_phone": "081-000-0000",
            ...
            # Create patient via the patients API
            async with self.session.post(
                ...
                if response.status in [200, 201]:
                    result = await response.json()
                    print(f"✅ Created patient record for {student.get('first_name')} {student.get('last_name')} (ID: {result.get('patient_id', 'N/A')})")
                    return True
    ...
    async def populate_patients_collection(self):
        """Populate the patients collection with student data"""
        ...
        for student in students:
            if await self.create_patient_record(student):
                success_count += 1
        
        print(f"\n📊 Summary:")
        print(f"   Total students: {len(students)}")
        print(f"   Patient records created: {success_count}")
        ...
        if success_count > 0:
            ...
            await self.test_patients_endpoint()
    
    async def test_patients_endpoint(self):
        """Test the patients endpoint to verify records were created"""
        try:
            headers = {"Authorization": f"Bearer {self.access_token}"}
            ...
            async with self.session.get(
                ...
                if response.status == 200:
                    ...
                    print(f"✅ Patients endpoint working! Found {len(patients)} patient records")
                    if patients:
                        print(f"   Sample patient: {patients[0].get('first_name')} {patients[0].get('last_name')}")
                    return True
                else:
                    print(f"❌ Patients endpoint failed: {response.status}")
                    return False
        except Exception as e:
...
async def main():
    """Main function"""
    print("🏥 EVEP Medical Portal - Populate Patients Collection")
    print("=" * 60)
    
    async with PatientsCollectionPopulator() as populator:
        await populator.populate_patients_collection()
