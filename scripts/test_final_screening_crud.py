#!/usr/bin/env python3
"""
Final Corrected CRUD Testing for Screening Endpoints
Tests with valid ObjectIds and all required fields
"""
...
class FinalScreeningCRUDTester:
    def __init__(self):
        self.session = None
        self.token = None
        self.test_results = {}
    ...
    async def test_screening_sessions_crud(self):
        ...
        # CREATE - Create a new screening session with valid ObjectId
        ...
        new_session = {
            ...
            "notes": "Test screening session with valid ObjectId"
        }
    ...
    async def test_mobile_screening_crud(self):
        ...
        # CREATE - Create a new mobile screening session with all required fields
        print("  🔸 Testing CREATE...")
        new_mobile_screening = {
            ...
            "equipment_calibration": "2025-01-01T00:00:00Z",  # Required field
            "notes": "Test mobile screening with all required fields"
        }
    ...
    async def test_school_screenings_crud(self):
        ...
        # Test CREATE for school screenings with all required fields
        print("  🔸 Testing CREATE...")
        new_school_screening = {
            "student_id": self.valid_object_ids["student_id"],
            "teacher_id": self.valid_object_ids["teacher_id"],
            "school_id": self.valid_object_ids["school_id"],  # Required field
            "screening_type": "comprehensive",
            "screening_date": datetime.now(timezone.utc).isoformat(),
            "notes": "Test school screening with all required fields"
        }
    ...
    async def test_va_screening_endpoints(self):
        """Test VA screening endpoints to understand their structure"""
        print("\n👁️ Testing VA Screening Endpoints...")
        
        # Test different VA screening endpoints
        va_endpoints = [
            "/api/v1/va-screening",
            "/api/v1/screenings/va",
            "/api/v1/va-screening/sessions",
            "/api/v1/va-screening/create"
        ]
        
        for endpoint in va_endpoints:
            print(f"  🔸 Testing {endpoint}...")
            result = await self.make_request("GET", endpoint)
            self.test_results[f"va_endpoint_{endpoint.replace('/', '_').replace('-', '_')}"] = result
            
            if result["success"]:
                print(f"    ✅ {endpoint} available: {result['status']}")
            else:
                print(f"    ❌ {endpoint} not available: {result['status']} - {result['data']}")
    ...
    async def run_all_tests(self):
        ...
        print("🚀 Starting Final Screening CRUD Tests")
        print("=" * 60)
        ...
        await self.test_va_screening_endpoints()
    ...
    def print_summary(self):
        ...
        print("\n" + "=" * 60)
        print("📊 FINAL SCREENING CRUD TESTS SUMMARY")
        print("=" * 60)
...
async def main():
    ...
    async with FinalScreeningCRUDTester() as tester:
